...
/* 보관에 같은 작품이 여러 줄 있던 것을 한 줄로 줄인다.
...
once(10, () => {
  ...
  const n = db.prepare(`DELETE FROM work WHERE state = 'watched' AND EXISTS (
    ...
    .run().changes;
  if (n) console.log(`  겹친 보관을 한 줄로: ${n}줄 거둠`);
});
...
/* **한 사람에게 같은 작품은 한 줄이다** — 휴지통만 빼고.
   ...
   한때 살아 있는 것과 보관을 따로 세어 저마다 한 줄씩 허락했다. 그래서 보관한
   작품을 친구 폴더에서 담아 오거나 주소로 다시 담으면 **줄이 하나 더 생겼다.** 목록에
   안 서던 때는 눈에 안 띄었는데, 보관을 페이지·폴더에 세우기 시작하자 같은 작품이
   두 장씩 보였다.
...
/** **이미 내가 들고 있는 같은 작품** — 살아 있든 보관이든. 없으면 null.
    ...
    **휴지통만 「없는」 것으로 본다.** 보관은 들고 있는 것이다 — 거기 있는 주소를
...
/** 남에게 넘기는 줄 — **상태는 들고 가지 않는다.**
    ...
    있는 한 편」이다. 그대로 넘기면 두 가지가 어긋난다: 받는 쪽의 「보관 보이기」
...
export function sharedView(ownerId: string, viewerId: string): { folders: Folder[]; works: Work[] } {
  ...
  /* **보관도 낸다.** 한때 살아 있는 것만 냈는데, 그러면 폴더에 그대로 두었는데도
