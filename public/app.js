...
/** 보관을 세울 수 있는 탭들 — 열쇠는 서버가 아는 것과 같다.
...
/** 보관한 작품을 이 탭에도 세우나 — 전체 설정에서 탭마다 켠다. */
const showsDone = where => (settings.doneIn ?? []).includes(where);

/** **그 탭이 세우는 작품들.**

    살아 있는 것은 늘 서고, 보관한 것은 그 탭을 켜 두었을 때만 함께 선다.
...
/* ── 작품 격자에서 여럿 고르기 ────────────────────────────
   ...
   하나만 둔다. 고른 것은 한꺼번에 보관이나 휴지통으로 보낸다 — 지우는 것이 아니라
    ...
    const draw = () => {
      ...
      sheet.querySelector(".sheet-body").addEventListener("click", ev => {
        ...
        if (ev.target.closest("[data-put-new]"))
          openFolderKind(f => f ? put(f.id, f.name) : draw());
      });
    };
    draw();
  }));
...
/** 이 날의 칸에 놓이는 작품들.
    ...
    **칸도 아래 단락과 같은 목록을 본다**(listedWorks). 한때 칸만 works 를 통째로 보면서
    내려 둔 작품을 「언제부터 언제까지 보던 작품」의 기록으로 놓았는데, 그러면 보관을
    캘린더에서 꺼 두어도 내린 그날 칸에는 그대로 남았다 — 껐는데 보이는 자리가 생긴다.
    기록은 보관 보관함이 이미 들고 있고, 켜면 앞으로의 날에도 선다.
...
function render() {
  writeHash();
  for (const [id, t] of [["tab-all", "all"], ["tab-cal", "cal"], ["tab-home", "home"], ["tab-lib", "lib"]])
  ...
  if (tab !== "home" && tab !== "all") workPickAt(null);

  let html = "";
  if (tab === "all") {
    /* **한 판에 다 세운다.** 페이지 탭은 같은 목록을 구간별로 갈라 놓은 것이고, 여기는
       가르지 않은 것이다 — 「어디 것이든 최근에 본 차례로」가 필요할 때가 있다.
       폴더 탭의 🗂 전체와 세우는 것이 같지만, 그건 폴더를 지나야 닿는 자리다.

       고르는 범위 이름을 페이지 탭과 **다르게** 둔다. 같은 이름을 쓰면 탭을 옮겨도
    ...
    ${st
      ? `<div class="link-row" style="margin-top:9px">
         ...
         ${keptSame(w) ? `<div class="rest" style="text-align:left;padding:6px 2px 0">
             ${whereKept(w)}.</div>` : ""}`
      : `<div class="link-row" style="margin-top:9px">
           <button class="btn" data-act="watched">${icon("check")} 보관</button>
...
function wirePicker(root, selected, onChange, reopen) {
  ...
  box.addEventListener("click", e => {
    ...
    if (b.dataset.pickerNew !== undefined) {
      openFolderKind(f => { if (f) selected.push(f.id); reopen(); });
      return;
    }
...
/** 새 폴더를 만들기 전에 **갈래부터 고른다.**

    한 창에서 다 하려면 「이 폴더로 무엇을 할 수 있나」를 켜고 끄는 칸이 늘 서 있어야 하고,
    그러면 만들고 나서 그 칸을 다시 열어 갈래를 바꿀 수 있다 — 함께 쓰자고 만든 폴더가
    편집 한 번에 아무나 담아가는 것이 되는 길이다. 앞에서 가르면 그 길이 없어진다.

    둘러보기(로그인 전)에는 공유할 상대가 없으므로 이 창을 건너뛰고 바로 일반 폴더로 간다. */
function openFolderKind(after) {
  if (guestMode()) return openFolderForm(null, after, "plain");
  const card = (kind, ico, title, desc) => `<button class="opt" data-kind="${kind}">
    <span class="ot"><b>${icon(ico)} ${title}</b><span>${desc}</span></span></button>`;
  openSheet(`
    ...
    ${/* **wide-only 를 붙이면 안 된다.** 그 갈래는 「마우스면 아래 버튼줄, 손가락이면
         ...
         마우스 기기에서만 서기 때문이다. 여기서는 늘 보이는 한 줄로 둔다. */""}
    <div class="link-row" style="margin-top:4px">
      <button class="btn" data-cancel>취소</button></div>`, { over: () => after(null) });
  const back = () => after(null);
  for (const b of sheet.querySelectorAll("[data-cancel]")) b.onclick = back;
  sheet.querySelector(".opts").addEventListener("click", e => {
    const b = e.target.closest("[data-kind]"); if (!b) return;
    // 갈래 창으로 되돌아올 길을 남긴다 — 잘못 골랐을 때 폴더 목록까지 나가지 않아도 되게
    openFolderForm(null, f => f ? after(f) : openFolderKind(after), b.dataset.kind);
  });
}
...
/** 보관함의 카드 한 장 — **여느 격자와 같은 카드**다.
    ...
    **줄에 있던 「복구」·「휴지통」 단추는 걷는다.** 카드에는 그것들이 설 자리가 없고,
    없어도 길은 둘 다 있다: 한 편은 카드를 눌러 작품 창에서(거기 그 둘이 이미 서 있다),
    여럿은 「선택」으로. 화면마다 다른 손짓을 두지 않는 편이 낫다.

    찍는 시각은 **내린 때**다. 목록이 내린 순서로 서 있으므로(byDone) 글씨도 그것을
    말해야 한다 — 자리와 글씨가 서로 다른 것을 말하면 안 된다. */
const archCard = w => arch.picked
  ? pickCardHtml(w, "data-pick", arch.picked)
  : workCard(w, ago(w.stateAt ?? w.addedAt));

const archGrid = list => `<div class="grid">${list.map(archCard).join("")}</div>`;
...
/** 이 작품과 **같은 url** 을 내가 이미 들고 있는가 — 있으면 그 상태, 없으면 null.
    ...
    보관도 자리를 차지한다 — 휴지통에서 복구했더니 목록에 두 장 서는 일이 없게.
...
function archBody() {
  ...
  return archGrid(byDone(g.items));
}
...
function drawArchive() {
  /* **보관함은 제 고르기(arch.picked)를 쓴다.** 카드는 workSel 하나만 보고 모양을
     정하므로(workCard), 다른 화면에서 고르던 것이 남아 있으면 여기 카드까지 그 모양이
     된다 — 화면을 떠나며 놓는 것이 맞다. */
  workSel = null; workSelKey = null;
  ...
  sheet.querySelector(".sheet-body").addEventListener("click", guard(async e => {
    /* **그 카드만 뒤집는다.** 격자를 통째로 다시 그리면 지금 짚고 있는 카드가 그 자리에서
       사라졌다 새로 생겨, 훑으며 톡톡 짚을 때 두 번째부터 헛손질이 된다
       (작품 격자 wireWorkPick 과 같은 이야기다). */
    const pk = e.target.closest("[data-pick]");
    if (pk && arch.picked) {
      const id = pk.dataset.pick;
      const on = !arch.picked.has(id);
      on ? arch.picked.add(id) : arch.picked.delete(id);
    ...
    if (bulk) {
      ...
      if (what !== "restore" && !await sureAbout(what, ids.length,
        ids.length === 1 ? works.find(w => w.id === ids[0])?.title : undefined)) return;
      return apply(ids, what);
    }

    /* 카드를 누르면 **작품 정보**가 열린다. 보관함에서도 「이게 뭐였더라」를 열어 볼 수
       있어야 하고, 한 편을 복구하거나 치우는 길도 거기 있다 — 줄에 달려 있던 단추를
       걷은 뒤로는 그것이 유일한 길이다. 고르는 중에는 열지 않는다: 그때 톡은 고르는 손짓이다. */
    if (arch.picked) return;
    const card = e.target.closest(".work[data-id]");
    if (card) openWork(card.dataset.id, drawArchive);
  }));
}
...
document.getElementById("tab-all").onclick = () => goTab("all");
